...
abstract class HttpBenchmarkBase {
    ...
    fun Javalin.attachEndpoints() {
        ...
        this.exception(Exception::class.java) { _, ctx -> ctx.status(500) }
        this.error(500) { it.result("Error") }

        // payload size scenarios
        this.get("/payload/empty") { it.result("") }
        this.get("/payload/text/100kb") { it.result(TEXT_100KB) }
        this.get("/payload/text/1mb") { it.result(TEXT_1MB) }

        // json serialization scenarios
        this.get("/payload/json/small") { it.json(JSON_SMALL) }
        this.get("/payload/json/100kb") { it.json(JSON_100KB) }
        this.get("/payload/json/1mb") { it.json(JSON_1MB) }

        // static-like raw byte responses
        this.get("/payload/static/100kb") { ctx ->
            ctx.contentType("application/octet-stream")
            ctx.result(ByteArrayInputStream(STATIC_100KB))
        }
        this.get("/payload/static/1mb") { ctx ->
            ctx.contentType("application/octet-stream")
            ctx.result(ByteArrayInputStream(STATIC_1MB))
        }

        // route-table scenarios
        registerRouteGroup(this, "10", 10)
        registerRouteGroup(this, "100", 100)
        registerRouteGroup(this, "1000", 1000)
    }

    private fun registerRouteGroup(app: Javalin, group: String, count: Int) {
        for (index in 0 until count) {
            app.get("/routes/$group/r$index") { it.result("route-$index") }
        }
    }
